...
# Script to build and run the Test MCP Mar19 USDC MCP Server locally in Docker
...
IMAGE_NAME="test-mcp-mar19-usdc-mcp-server"
CONTAINER_NAME="test-mcp-mar19-usdc-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Test MCP Mar19 USDC MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Test MCP Mar19 USDC MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Test MCP Mar19 USDC MCP Server is running!${NC}"
echo
